...
/* SafeHtmlChecker - checks HTML against a subset of 
   ...
   Updated 15th September 2003: Added extra <? and <script filters.
	   ...
	   Updated by Peter Keane 15 March 2008
	   for PHP5 & DASeification
...
define('E_BLOCK_CONTENTS', 'dl ul ol blockquote p h1 h2 h3 h4 h5 h6');
define('E_FLOW_CONTENTS', E_BLOCK_CONTENTS.' '.E_INLINE_CONTENTS);

class Dase_SafeHtmlChecker 
{
	...
	function getResultsAsJson() 
	{
		...
		if ($this->isOK()) {
			...
			foreach ($this->errors as $error) {
				$results[] = strip_tags($error);
			}
		}
		return Dase_Json::get($results);
	}
}
