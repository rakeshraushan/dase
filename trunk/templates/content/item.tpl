...
<div class="viewitem" style="width:{$item->viewitem->width}px;">
<a href="{$collection->ascii_id}/{$item->serial_number}">
<img src="{$item->viewitem->url}" alt="file this in w/ simple title"/>
</a>
...
<div class="metadata">
<dl>
{foreach item=value from=$item->values}
<dt>{$value->attribute_name}</dt>
<dd>{$value->value_text}</dd>
{/foreach}
</dl>
