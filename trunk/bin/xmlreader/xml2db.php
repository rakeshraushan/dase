...
foreach (new DirectoryIterator(XML_DIR) as $colldir) {
	$xml_file = XML_DIR. '/'."$colldir/collection.xml";
	$items_xml_dir = XML_DIR. '/'."$colldir/items";
	if (is_file($xml_file)) {
		$coll_id = getCollectionXml($xml_file);
		if (is_dir($items_xml_dir)) {
			getItemXml($items_xml_dir,$coll_id);
		}
	}
}
