...
class Upload 
{
	function getUploadedFile($file) {
		$this->original_name = trim($file['name']); //orig filename
		$this->tmp_path = $file['tmp_name']; //temp path
		$this->upload_path = $file['tmp_name']; //temp path
		$this->mime_type = $file['type']; //mime_type
		$this->file_size = $file['size']; //size
		$this->error = $file['error']; //size
		$this->file_info = getimagesize($this->tmp_path);
		$types_map = array(
		...
		$this->valid_types = array_keys($types_map);
		$this->size = $types_map[$this->mime_type]['size'];
		$this->ext = $types_map[$this->mime_type]['ext'];
	}
